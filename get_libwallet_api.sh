...
git -C $OSCILLATE_DIR checkout release-v0.5.1
...
#git -C $OSCILLATE_DIR submodule update --remote

#update submodules
#git -C $OSCILLATE_DIR submodule update --init --force external/rapidjson
#git -C $OSCILLATE_DIR submodule update --init --force external/unbound
#git -C $OSCILLATE_DIR submodule update --init --force external/miniupnp
...
## LINUX 64
    ...
    if [ "$ANDROID" == true ]; then
        ...
        cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D ARCH="armv7-a" -D ANDROID=true -D BUILD_GUI_DEPS=ON -D USE_LTO=OFF -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$OSCILLATE_DIR"  ../..
...
## Windows is always static to work outside msys2
    ...
    cmake -D CMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE -D STATIC=ON -D BOOST_ROOT="$BOOST_ROOT" -D ARCH="x86-64" -D BUILD_64=ON -D BUILD_GUI_DEPS=ON -D INSTALL_VENDORED_LIBUNBOUND=ON -D CMAKE_INSTALL_PREFIX="$OSCILLATE_DIR" -G "MSYS Makefiles" -D CMAKE_TOOLCHAIN_FILE=../../cmake/64-bit-toolchain.cmake -D MSYS2_FOLDER=c:/msys64 ../..
