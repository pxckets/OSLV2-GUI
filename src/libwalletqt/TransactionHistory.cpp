...
QList<TransactionInfo *> TransactionHistory::getAll(quint32 accountIndex) const
{
    ...
    QDateTime firstDateTime = QDateTime(QDate(2018, 6, 14)); // the genesis block
...
{
    m_firstDateTime  = QDateTime(QDate(2018, 6, 14)); // the genesis block
...
QString TransactionHistory::writeCSV(quint32 accountIndex, QString out)
{
    ...
    QString fn = QString(QString("%1/Oscillate-txs_%2.csv").arg(out, QString::number(now / 1000)));
